...
namespace SharpSharp.Pipeline {
	internal sealed class Processor {
		...
		public void Process(ImageSource imageSource, PipelineBaton baton) {
			...
			if(baton.ToTargetOptions.HasValue()) {
				PotentiallySaveJpegTarget(baton, image, imageType);
				PotentiallySavePngTarget(baton, image, imageType);
				PotentiallySaveWebpTarget(baton, image, imageType);
		...
		private static bool HasNoOutputSelected(PipelineBaton baton) =>
			baton.GifOptions == null && baton.HeifOptions == null && baton.JpegOptions == null &&
			baton.PngOptions == null && baton.RawOptions == null && baton.TiffOptions == null && baton.WebpOptions == null;
		...
		private static void PotentiallySaveGifBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(go == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.Gif) {
				go = new GifOptions();
			}
			else {
				return;
		...
		private static void PotentiallySaveGifTarget(PipelineBaton baton, Image image, ImageType imageType) {
			...
			throw new SharpSharpException("Cannot save GIF to target, it's unsupported by libvips.");
		}
		...
		private static void PotentiallySaveHeifBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(ho == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.Heif) {
				ho = new HeifOptions();
			}
			else {
				return;
		...
		private static void PotentiallySaveJpegBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(jo == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.Jpeg) {
				jo = new JpegOptions();
			}
			else {
				return;
		...
		private static void PotentiallySavePngBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(po == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && (imageType == ImageType.Png || (imageType == ImageType.Gif && SupportsGifOutput) || imageType == ImageType.Svg)) {
				po = new PngOptions();
			}
			else {
				return;
		...
		private static void PotentiallySaveRawBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(ro == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.Raw) {
			}
			else {
				return;
			}
		...
		private static void PotentiallySaveRawTarget(PipelineBaton baton, Image image, ImageType imageType) {
			...
			image.WriteToTarget(to.Target, baton.OutputInfo.Format);
		...
		private static void PotentiallySaveTiffBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(to == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.Tiff) {
				to = new TiffOptions();
			}
			else {
				return;
		...
		private static void PotentiallySaveWebpBuffer(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(wo == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && imageType == ImageType.WebP) {
				wo = new WebpOptions();
			}
			else {
				return;
		...
		private static void PotentiallySaveWebpFile(PipelineBaton baton, Image image, ImageType imageType) {
			...
			else if(wo == null && HasNoOutputSelected(baton) && MightMatchInput(formatOut) && Common.IsWebp(fo.FilePath)) {
				wo = new WebpOptions();
			}
